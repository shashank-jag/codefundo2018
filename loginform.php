...
if(isset($_POST['id'])&&!empty($_POST['Password'])){
	...
	if(!empty($username)&&!empty($password)){
		$query="SELECT * FROM `user` where `username`='".$_POST['id']."' AND `password`='".$_POST['Password']."'";
		...
		if($run=mysqli_query($link,$query)){
			...
			if(mysqli_num_rows($run)==0){
				...
				if(strcmp($row_login['category'],'Consumer')==0){
					header('Location: 14_consumerBuy.php');
				}
				else if(strcmp($row_login['category'],'Farmer')==0){
